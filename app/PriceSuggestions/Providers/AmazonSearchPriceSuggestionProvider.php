...
class AmazonSearchPriceSuggestionProvider implements IngredientPriceSuggestionProviderInterface
{
    public function suggest(Ingredient $ingredient, PriceCategory $priceCategory): array
    {
        if ($priceCategory->suggestion_provider !== 'amazon-search') {
            return [];
        }

        $config = is_array($priceCategory->suggestion_config) ? $priceCategory->suggestion_config : [];
        $urlTemplate = trim((string) ($config['url_template'] ?? 'https://www.amazon.de/s?k={query}'));
        ...
        return $this->findCandidates($html, $ingredient, $priceCategory, $query);
    }
    ...
    private function fetchHtml(string $urlTemplate, string $query, Ingredient $ingredient, PriceCategory $priceCategory): ?string
    {
        ...
        try {
            ...
            Log::warning('Amazon price suggestion provider failed', [
    ...
    private function findCandidates(string $html, Ingredient $ingredient, PriceCategory $priceCategory, string $searchQuery): array
    {
        $xpath = $this->createXPath($html);
        if ($xpath === null) {
            return [];
        ...
        foreach ($nodes as $index => $node) {
            $linkNode = $xpath->query('.//*[@data-cy="title-recipe"]//a[contains(@class, "a-text-normal")][@href][1]', $node)?->item(0)
                ?? $xpath->query('.//*[@data-cy="title-recipe"]//a[contains(@class, "s-link-style")][@href][1]', $node)?->item(0)
                ?? $xpath->query('.//*[@data-cy="title-recipe"]//a[@href][1]', $node)?->item(0)
                ?? $xpath->query('.//h2//a[@href][1]', $node)?->item(0)
                ?? $xpath->query('.//a[contains(@class, "a-text-normal")][@href][1]', $node)?->item(0)
                ?? $xpath->query('.//a[contains(@class, "s-link-style")][@href][1]', $node)?->item(0);
            $imageNode = $xpath->query('.//img[contains(@class, "s-image")][1]', $node)?->item(0);
            $priceTextNode = $xpath->query('.//*[contains(@class, "a-price")]//*[contains(@class, "a-offscreen")][1]', $node)?->item(0);

            if ($linkNode === null || $priceTextNode === null) {
                continue;
            }

            $name = trim((string) preg_replace('/\s+/u', ' ', $linkNode->textContent ?? ''));
            $priceText = trim((string) preg_replace('/\s+/u', ' ', $priceTextNode->textContent ?? ''));
            $url = $this->normalizeAmazonUrl($linkNode->attributes?->getNamedItem('href')?->nodeValue);
            ...
            $priceMinor = $this->parsePriceToMinor($priceText, $priceCategory->currency);
            if ($priceMinor === null) {
                continue;
            }
            ...
            $candidates[] = [
                ...
                'candidate' => new IngredientPriceSuggestionCandidate(
                    ...
                    amount: $amount,
                    units: $units,
                    confidence: min(0.95, max(0.3, $score / 20)),
                    meta: [
                        'reason' => 'Fetched from Amazon search results',
                        'image_url' => $imageUrl,
                        'result_index' => $index,
                        'score' => $score,
                    ],
    ...
    private function parsePriceToMinor(string $priceText, string $currency): ?int
    {
        if (preg_match('/([0-9]+(?:[\.,][0-9]{2})?)/u', str_replace(['€', 'EUR'], '', $priceText), $matches) !== 1) {
            return null;
        }
    ...
    private function normalizeAmazonUrl(?string $url): ?string
    {
        ...
        if (str_starts_with($url, '/')) {
            return 'https://www.amazon.de' . $url;
        }
